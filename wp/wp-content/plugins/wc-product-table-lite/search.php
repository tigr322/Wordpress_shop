...
function wcpt_search($filter_info, $post__in = array())
{
  ...
  if (!empty($filter_info['use_default_search'])) {
    ...
    foreach ($search_terms as $term) { // @todo - replace with wc search 
  ...
  return wcpt_search__combine(apply_filters('wcpt_search_ids', $search_ids, $filter_info, $keyword_phrase, $keywords), $post__in);
}
